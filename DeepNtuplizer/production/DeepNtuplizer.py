...
options.register("phase2", True, VarParsing.VarParsing.multiplicity.singleton, VarParsing.VarParsing.varType.bool, "apply jet selection for phase 2 (currently sets JetEtaMax to 4.0 and picks slimmedJetsPuppi as jet collection)")
if hasattr(sys, "argv"):
...
process.options = cms.untracked.PSet(allowUnscheduled=cms.untracked.bool(True), wantSummary=cms.untracked.bool(False))
...
process.looseIVFinclusiveCandidateVertexFinder.vertexMinDLen2DSig = cms.double(0.0)
process.looseIVFinclusiveCandidateVertexFinder.vertexMinDLenSig = cms.double(0.0)
...
if options.phase2:
    process.deepntuplizer.jetAbsEtaMax = cms.double(4.0)
...
process.p = cms.Path(process.QGTagger + process.genJetSequence * process.deepntuplizer, process.tsk)
